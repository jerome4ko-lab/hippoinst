...
_ROBOT_PRIORITY = """
주제 우선순위(로봇 일반 — 휴머노이드 외 arm-form / AI 로봇 / 스포츠 로봇 모두 포함):
- 하드웨어 성능 변화 (속도·적재량·관절수·자유도 등 수치)
- 실제 스포츠·가사·군사·산업 현장에서의 시연
- AI 로봇이 스포츠·게임에서 인간 프로 격파 (탁구·체스·바둑·테니스 등)
- 대규모 생산·상용화 돌파구
"""
...
⚠️ links 토픽 일치 — 절대 위반 금지:
- 각 항목의 links 배열에 들어가는 URL은 해당 항목의 title·summary 가 가리키는 **바로 그 사건**을 직접 보도하는 기사여야 합니다.
- 같은 대주제(예: "휴머노이드 로봇")라는 이유만으로 다른 사건의 기사를 끼워 넣지 마세요.
  · 잘못된 예: title이 "중국 휴머노이드 풀마라톤 완주"인데 links에 펜타곤 계약 / Figure 02 데모 / Tesla Optimus 기사 URL을 넣는 것.
  · 올바른 예: 마라톤 결승 사진·기록·주최사를 보도하는 기사만 채우고, 모자라는 슬롯은 "검색 필요"로 둔다.
- 슬롯이 모자라더라도 무리해서 카테고리만 같은 URL로 채우지 마세요. 빈 슬롯은 "검색 필요" 가 정답입니다.
- 같은 토픽 풀의 URL을 여러 항목이 공유하는 일이 없도록, item 별로 독립 검색·검증하세요.
"""

_TOPIC_QUERY = {
    "robot": (
        "로봇 / robot 최근 한 달 핵심 뉴스 — "
        "휴머노이드·산업로봇·AI 로봇·스포츠 데모(탁구·체스 등) 모두 포함"
    ),
...
def search_news(topic: str, *, limit: int = 7, **_ignored) -> list[dict]:
    ...
    # web_search 응답에 섞이는 <cite index="...">...</cite> 태그 제거
    for it in items:
        for k in ("title", "keyword", "summary"):
            if isinstance(it.get(k), str):
                it[k] = _strip_cite_tags(it[k])
...
def _strip_cite_tags(text: str) -> str:
    """<cite index="...">...</cite> 태그만 제거하고 안의 텍스트는 보존.
    동시에 태그 제거로 생긴 중복 공백을 정리.
    """
    s = _CITE_RE.sub("", text)
    s = re.sub(r"[ \t]+([.,;:!?])", r"\1", s)  # 태그 떨어지며 생긴 ' .' 같은 것 정리
    s = re.sub(r"[ \t]{2,}", " ", s)
    return s.strip()
