...
AI·로봇·미래 신기술 채널을 위한 45초 쇼츠 스크립트를 작성합니다.
...
규칙:
- narration은 12~20줄, 한 줄당 20~35자 (읽기 속도 1.2배 기준)
...
def generate_script_from_articles(articles: list[str]) -> dict:
    ...
    prompt = f"""다음 기사들을 읽고 가장 임팩트 있는 핵심 사실 하나를 골라 45초 쇼츠 스크립트를 작성해주세요.
...
def generate_script(topic: str) -> dict:
    prompt = f"""다음 주제로 45초 쇼츠 스크립트를 작성해주세요.
주제: {topic}
